...
Research into agent file formats (skills, rules, memory, hooks, agents, plugins), invocation methods, context loading strategies, and format differences across AI coding tools — informing skync's connector architecture.
...
OpenClaw has the richest file taxonomy — 8 separate config files loaded at session start into the system prompt. This gives fine-grained control but means more files for skync to discover and potentially sync.
...
PicoClaw prioritizes extreme minimalism. For skync, it would likely be an MCP target rather than a skill directory target.
...
Goose is built entirely around MCP — extensions are MCP servers, not skill directories. For skync, Goose is exclusively an MCP target.
...
### Migration Pain
No tool provides automatic format conversion. Moving from `.cursorrules` to SKILL.md, or from Windsurf rules to anything else, requires manual work. This is the gap skync aims to fill.

---

## 19. What This Means for skync

### Format Families to Support

Based on this research, skync's connector architecture needs to handle four format families:
...
4. **MCP config for tool-based targets** — Tools that consume skills via MCP get a config entry pointing at the skync MCP server rather than direct file access. Goose is the primary MCP-only target.
...
Tool-specific skill extensions are lost in translation — skync should preserve them in metadata when syncing between tools of the same type, but can safely drop them when translating to a different format family.
...
Everything else (hooks, agents, plugins, memory) remains fragmented with no signs of standardization. For skync, this means the v1 focus on skills + MCP + instruction files covers the portable surface area. Extended structures would require per-tool connectors with no format translation possible.
