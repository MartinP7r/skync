...
skync has two layers of tests: **unit tests** co-located with each module, and **integration tests** that exercise the compiled binary end-to-end. All tests run in CI on both Ubuntu and macOS.
...
These compile the `skync` binary and run it as a subprocess using `assert_cmd`. They verify the full CLI flow: argument parsing, config loading, pipeline execution, and output formatting.

```mermaid
graph LR
    subgraph Integration["tests/cli.rs"]
        CMD["assert_cmd<br/>spawns skync binary"]
...
| `distribute_mcp_creates_config` | MCP method creates `.mcp.json` with skync entry |
| `distribute_mcp_preserves_existing_servers` | Adding skync entry doesn't clobber existing servers |
...
Each test compiles and runs the `skync` binary in a temp directory with a custom config.
...
- Tests never touch the real `~/.config/skync/` or `~/.local/share/skync/`
...
cargo test -p skync -- discover::tests

# Only integration tests
cargo test -p skync --test cli
